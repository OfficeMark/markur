...
export const PinMarker = forwardRef<HTMLButtonElement, PinMarkerProps>(function PinMarker(
  ...
  const lockSuffix = unlocked ? ', unlocked — drag to move' : '';
  return (
    <button
      ref={ref}
      type="button"
      ...
      onPointerDown={(e) => {
        ...
        e.stopPropagation();
        if (unlocked) onPointerDownDrag?.(e);
      }}
      onClick={(e) => {
        e.stopPropagation();
        onClick?.();
      ...
      className={cn(
        ...
        FILL_COLOR[status],
        selected && 'scale-110',
        unlocked && 'cursor-grab touch-none ring-4 ring-waymarks-gold/40 ring-offset-1',
        unlocked && 'after:pointer-events-none after:absolute after:-inset-1 after:animate-pulse after:rounded-full after:border-2 after:border-dashed after:border-waymarks-gold',
        !unlocked && selected && 'ring-4 ring-waymarks-gold/40',
