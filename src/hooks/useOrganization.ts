...
export function useOrgSubscription(): OrgSubscriptionState {
  ...
  // Prefer the org's subscription row from the app_boot bundle (already loaded
  // app-wide) so the lockout gating doesn't fire its own organizations fetch.
  ...
  // org — e.g. a locked org omitted from app_boot. The organizations SELECT
  // policy is authenticated-readable (not gated by user_can), so the lockout
  // path still resolves for a locked org's admin.
