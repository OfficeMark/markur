-- phase2c: super_admin grants are only blanket when scope_type = 'global'.
-- Closes the footgun where an org-scoped super_admin grant silently granted
-- all-tenant access. Aligns user_can / user_can_view_asset / user_in_org with
-- user_in_building, which already gated on global scope.
-- CREATE OR REPLACE preserves OIDs; all RLS policy references stay intact.
...
begin
  ...
  -- building_admin / auditor: defer to user_can('view','floor',...) (their existing path).
  if exists (
    select 1 from public.access_grants
    where user_id = v_user
...
CREATE OR REPLACE FUNCTION private.user_in_org(p_org_id uuid)
 RETURNS boolean
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'private', 'public'
